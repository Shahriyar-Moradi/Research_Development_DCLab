...
function renderDetail(run) {
  const events=run.events, trials=[...new Map(events.filter(e=>e.kind==='trial').map(e=>[e.payload.id,e.payload])).values()];
  ...
  $('run-detail').innerHTML=`
    ...
    <div class="run-grid"><div>
      ...
      ${synthesis?`<div class="card panel evidence"><h2>Research synthesis</h2><p>${esc(synthesis.summary)}</p>${synthesis.lessons.map(l=>`<h3>${esc(l.claim)}</h3><p>${esc(l.scope)}</p><p><b>Counterevidence:</b> ${esc(l.counterevidence)}</p>`).join('')}<details data-key="unanswered"><summary>Still unanswered</summary>${list(synthesis.unanswered_questions)}</details></div>`:''}
