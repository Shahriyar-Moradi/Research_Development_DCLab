...
from .archive import append_event_file, write_manifest, write_run_config, write_studio_index
...
class Store:
    ...
    def create(self, run_id, config):
        ...
        write_run_config(self, run_id)
        write_studio_index(self)
        return run
    ...
    def event(self, run_id, kind, payload):
        ...
        with self.connect() as db:
            cur = db.execute(
                "INSERT INTO events(run_id,kind,created,payload) VALUES (?,?,?,?)",
                (run_id, kind, created, json.dumps(payload)),
            )
            seq = cur.lastrowid
        append_event_file(self, run_id, seq, kind, created, payload)
        return seq
    ...
    def events(self, run_id):
        with self.connect() as db:
            return [
                ...
                for seq, kind, created, payload in db.execute(
                    "SELECT seq,kind,created,payload FROM events WHERE run_id=? ORDER BY seq",
                    (run_id,),
                )
            ]
    ...
    def knowledge(self, datasets=None):
        ...
        for run in self.list():
            if datasets and not set(datasets) & set(run["config"]["datasets"]):
                continue
                        ...
                        result.append(
                            {
                                **lesson,
                                ...
                                "status": "provisional_llm_interpretation",
    ...
    def export(self, run_id):
        run = self.get(run_id)
        from .archive import run_label

        label = run_label(run)
        # Explicit decisions and tool evidence, not private reasoning traces.
        return {
            "schema_version": 2,
            "purpose": "Research trajectory for review before future training",
            ...
            "review_requirements": [
                ...
                "Check dataset licenses and privacy",
                "Assign all trajectories sharing a dataset/source to the same training or evaluation split",
                "Remove prompt injection and secret-like text",
                "Retain failures and counterevidence",
            ],
            "run": run,
            "events": [e for e in self.events(run_id) if e["kind"] not in ("llm_request",)],
        }

    def finalize_archive(self, run_id: str) -> dict:
        """Refresh trajectory, manifest, and studio index after a run finishes."""
        from .archive import archive_run

        manifest = archive_run(self, run_id, reset_snapshots=True)
        write_studio_index(self)
        write_manifest(self, run_id)
        return manifest
