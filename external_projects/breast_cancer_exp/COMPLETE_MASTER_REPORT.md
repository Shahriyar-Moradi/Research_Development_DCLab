# Breast Cancer Wisconsin (Diagnostic) — Complete Master Report

HyperAck-style end-to-end study for **Breast Cancer Wisconsin (Diagnostic)**.

## Executive summary

| Item | Value |
|---|---|
| Best safe ROC-AUC | **1.0000** (`fe_ratios`) |
| Best unsafe ROC-AUC | **1.0000** (`fe_ratios`) |
...
Safe baseline / FE ladder    ──▶  see FEATURE_ENGINEERING_REPORT.md
        ...
        ▼
Safe optimized champion      ──▶  1.0000  (fe_ratios)
```
...
```bash
.venv/bin/python general_pipeline/playbook/run_playbook.py --dataset breast_cancer
