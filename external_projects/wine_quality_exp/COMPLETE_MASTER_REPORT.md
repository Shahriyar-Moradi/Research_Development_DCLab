# Wine Quality (Binary) — Complete Master Report

HyperAck-style end-to-end study for **Wine Quality (Binary)**.

## Executive summary

| Item | Value |
|---|---|
| Best safe ROC-AUC | **0.8968** (`stacking_ensemble`) |
| Best unsafe ROC-AUC | **0.8968** (`stacking_ensemble`) |
| Best optimization method | `oof_stacking` |
| Leakage present | False |
| Leakage columns | `[]` |

```
Unsafe ceiling (if leakage)  ──▶  0.8968
        │ drop leakage
        ▼
Safe baseline / FE ladder    ──▶  see FEATURE_ENGINEERING_REPORT.md
        │ RandomizedSearch / ensembles
        ▼
Safe optimized champion      ──▶  0.8968  (stacking_ensemble)
```
...
```bash
.venv/bin/python general_pipeline/playbook/run_playbook.py --dataset wine_quality
