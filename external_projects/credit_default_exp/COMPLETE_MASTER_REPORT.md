# Default of Credit Card Clients — Complete Master Report

HyperAck-style end-to-end study for **Default of Credit Card Clients**.

## Executive summary

| Item | Value |
|---|---|
| Best safe ROC-AUC | **0.8005** (`stacking_ensemble`) |
| Best unsafe ROC-AUC | **0.8005** (`stacking_ensemble`) |
| Best optimization method | `oof_stacking` |
| Leakage present | False |
| Leakage columns | `[]` |

```
Unsafe ceiling (if leakage)  ──▶  0.8005
        │ drop leakage
        ▼
Safe baseline / FE ladder    ──▶  see FEATURE_ENGINEERING_REPORT.md
        │ RandomizedSearch / ensembles
        ▼
Safe optimized champion      ──▶  0.8005  (stacking_ensemble)
```
