# Mushroom — Complete Master Report

HyperAck-style end-to-end study for **Mushroom**.

## Executive summary

| Item | Value |
|---|---|
| Best safe ROC-AUC | **1.0000** (`hist_gradient_boosting`) |
| Best unsafe ROC-AUC | **1.0000** (`hist_gradient_boosting`) |
| Best optimization method | `mutual_info_selection` |
| Leakage present | False |
| Leakage columns | `[]` |

```
Unsafe ceiling (if leakage)  ──▶  1.0000
        │ drop leakage
        ▼
Safe baseline / FE ladder    ──▶  see FEATURE_ENGINEERING_REPORT.md
        │ RandomizedSearch / ensembles
        ▼
Safe optimized champion      ──▶  1.0000  (hist_gradient_boosting)
```
...
```bash
.venv/bin/python general_pipeline/playbook/run_playbook.py --dataset mushroom
