# Spambase — Complete Master Report

HyperAck-style end-to-end study for **Spambase**.

## Executive summary

| Item | Value |
|---|---|
| Best safe ROC-AUC | **0.9878** (`baseline_lightgbm`) |
| Best unsafe ROC-AUC | **0.9878** (`baseline_lightgbm`) |
| Best optimization method | `oof_stacking` |
| Leakage present | False |
| Leakage columns | `[]` |

```
Unsafe ceiling (if leakage)  ──▶  0.9878
        │ drop leakage
        ▼
Safe baseline / FE ladder    ──▶  see FEATURE_ENGINEERING_REPORT.md
        │ RandomizedSearch / ensembles
        ▼
Safe optimized champion      ──▶  0.9878  (baseline_lightgbm)
```
