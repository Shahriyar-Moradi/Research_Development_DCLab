# Online Shoppers Purchasing Intention — Complete Master Report

HyperAck-style end-to-end study for **Online Shoppers Purchasing Intention**.

## Executive summary

| Item | Value |
|---|---|
| Best safe ROC-AUC | **0.7737** (`tuned_xgboost`) |
| Best unsafe ROC-AUC | **0.9311** (`tuned_lightgbm`) |
| Best optimization method | `RandomizedSearchCV_roc_auc` |
| Leakage present | True |
| Leakage columns | `['PageValues']` |

```
Unsafe ceiling (if leakage)  ──▶  0.9311
        │ drop leakage
        ▼
Safe baseline / FE ladder    ──▶  see FEATURE_ENGINEERING_REPORT.md
        │ RandomizedSearch / ensembles
        ▼
Safe optimized champion      ──▶  0.7737  (tuned_xgboost)
```
...
```bash
.venv/bin/python general_pipeline/playbook/run_playbook.py --dataset online_shoppers
