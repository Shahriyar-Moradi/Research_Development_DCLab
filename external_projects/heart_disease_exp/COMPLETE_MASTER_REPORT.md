# Heart Disease — Complete Master Report

HyperAck-style end-to-end study for **Heart Disease**.

## Executive summary

| Item | Value |
|---|---|
| Best safe ROC-AUC | **0.9740** (`hist_gradient_boosting`) |
| Best unsafe ROC-AUC | **0.9740** (`hist_gradient_boosting`) |
| Best optimization method | `RandomizedSearchCV_roc_auc` |
| Leakage present | False |
| Leakage columns | `[]` |

```
Unsafe ceiling (if leakage)  ──▶  0.9740
        │ drop leakage
        ▼
Safe baseline / FE ladder    ──▶  see FEATURE_ENGINEERING_REPORT.md
        │ RandomizedSearch / ensembles
        ▼
Safe optimized champion      ──▶  0.9740  (hist_gradient_boosting)
```
...
```bash
.venv/bin/python general_pipeline/playbook/run_playbook.py --dataset heart_disease
