# Adult (Census Income) — Complete Master Report

HyperAck-style end-to-end study for **Adult (Census Income)**.

## Executive summary

| Item | Value |
|---|---|
| Best safe ROC-AUC | **0.9261** (`baseline_lightgbm`) |
| Best unsafe ROC-AUC | **0.9261** (`baseline_lightgbm`) |
| Best optimization method | `RandomizedSearchCV_roc_auc` |
| Leakage present | False |
| Leakage columns | `[]` |

```
Unsafe ceiling (if leakage)  ──▶  0.9261
        │ drop leakage
        ▼
Safe baseline / FE ladder    ──▶  see FEATURE_ENGINEERING_REPORT.md
        │ RandomizedSearch / ensembles
        ▼
Safe optimized champion      ──▶  0.9261  (baseline_lightgbm)
```
