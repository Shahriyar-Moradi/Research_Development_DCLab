"""Run the same HyperAck experiments with leakage-safe features only (no final fares)."""
...
HYPERACK_ROOT = PROJECT_ROOT / "hyperack_exp"
SAFE_RESULTS = SAFE_ROOT / "results"
SAFE_RESULTS.mkdir(parents=True, exist_ok=True)

sys.path.insert(0, str(HYPERACK_ROOT))

from shared.protocol import (  # noqa: E402
    evaluate,
    load_clean_df,
    make_xy as _make_xy_unsafe,
...
# Force leakage-safe features everywhere: drop final_* fares.
UNSAFE_COLS = {"final_customer_fare", "final_biker_fare"}


def make_xy(df, **kwargs):
    kwargs["include_final_fares"] = False
    X, y = _make_xy_unsafe(df, **kwargs)
    drop = [c for c in X.columns if c in UNSAFE_COLS]
    if drop:
        X = X.drop(columns=drop)
    return X, y


def save_result(exp_id, name, strategy, metrics, *, best_model, notes, feature_count):
    strategy = f"[SAFE] {strategy}"
    notes = f"{notes} | Leakage-safe: final fares dropped."
    return _save_result(
        exp_id,
        name,
        strategy,
...
def run_11(train_df, test_df):
    # TabPFN needs interactive license; keep slot but skip cleanly in safe suite.
    import json
    payload = {
        "exp_id": "11",
        "name": "tabpfn",
        "strategy": "[SAFE] TabPFN skipped (license/token)",
        "primary_metric": "roc_auc",
        "metrics": {"roc_auc": None, "status": "skipped_license"},
        "best_model": "TabPFNClassifier",
        "feature_count": 0,
        "notes": "Skipped in safe suite for the same license reason as unsafe suite.",
    }
    (SAFE_RESULTS / "11_tabpfn.json").write_text(json.dumps(payload, indent=2) + "\n")
    return None

def run_11_disabled(train_df, test_df):
...
def main():
    ...
    log(f"[SAFE LEAKAGE] train={len(train_df)} test={len(test_df)} results={SAFE_RESULTS}")
    # Prove final fares are absent on a sample matrix
    _X, _ = make_xy(train_df, pricing=True, time_features=True, geo=True)
    leaked = [c for c in _X.columns if "final_" in c]
    log(f"Safe feature count sample={_X.shape[1]}; forbidden still present={leaked}")
